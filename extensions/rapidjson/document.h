...
public:
	...
	*/
	...
	{  
		...
		if (rhs.IsArray())
		{
			this->data_.a.elements = (GenericValue*)allocator.Malloc(rhs.data_.a.capacity * sizeof(GenericValue));
			for (GenericValue *p = this->data_.a.elements, *v = rhs.data_.a.elements; 
				v != rhs.data_.a.elements + rhs.data_.a.size; 
				++v, ++p) 
			{
				new(p)GenericValue(*v, allocator);
			}
		}
		else if (rhs.IsObject())
		{
			this->data_.o.members = (Member*)allocator.Malloc(rhs.data_.o.capacity * sizeof(Member));
			for (Member *p = this->data_.o.members, *m = rhs.data_.o.members;
				m != rhs.data_.o.members + rhs.data_.o.size;
				++m, ++p) 
			{
				new(&p->name)GenericValue(m->name, allocator);
				new(&p->value)GenericValue(m->value, allocator);
			}

		}
		else if (rhs.IsString())
		{
			new (this)GenericValue(rhs.data_.s.str, rhs.data_.s.length, allocator);
		}
	}
...
\endcode
	...
	*/
	...
	operator const Ch*() const	{ return GetString(); }
	//add by Whislly end
